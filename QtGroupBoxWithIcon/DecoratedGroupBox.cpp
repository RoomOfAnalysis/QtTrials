...
StyleOptionDecoratedGroupBox::StyleOptionDecoratedGroupBox()
    : QStyleOptionGroupBox(), offset(0), leftSpace(0), rightSpace(0)
{
}

CustomStyle::CustomStyle(): QCommonStyle() {}

void CustomStyle::drawComplexControl(QStyle::ComplexControl cc, const QStyleOptionComplex* opt, QPainter* p,
                                     const QWidget* widget) const
{
    if (const StyleOptionDecoratedGroupBox* groupBox = qstyleoption_cast<const StyleOptionDecoratedGroupBox*>(opt))
    {
        ...
        if (!groupBox->pixmap.isNull())
        {
            ...
            pixmapRectWidth = groupBox->leftSpace + groupBox->pixmap.width() + groupBox->rightSpace;
        ...
        if (!groupBox->pixmap.isNull())
        {
            ...
            proxy()->drawItemPixmap(p, QRect(decorationOffset, 0, pixmapRectWidth, pixmapHeight), Qt::AlignCenter,
                                    groupBox->pixmap);
        }
        ...
        if ((groupBox->subControls & QStyle::SC_GroupBoxLabel) && !groupBox->text.isEmpty())
        {
            ...
            if (textColor.isValid()) p->setPen(textColor);
            int alignment = int(groupBox->textAlignment);
            if (!proxy()->styleHint(QStyle::SH_UnderlineShortcut, opt, widget)) alignment |= Qt::TextHideMnemonic;
            proxy()->drawItemText(p, textRect, Qt::TextShowMnemonic | Qt::AlignHCenter | alignment, groupBox->palette,
                                  groupBox->state & State_Enabled, groupBox->text,
...
DecoratedGroupBox::DecoratedGroupBox(const QString& title, QWidget* parent)
    : QGroupBox(title, parent), m_offset(5), m_leftSpace(3), m_rightSpace(3)
{
}
