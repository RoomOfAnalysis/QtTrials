...
//
DataQueryEngine::DataQueryEngine(QObject* parent): QObject(parent), m_queryTimer(new QTimer(this))
{
...
//================================================================================
void DataQueryEngine::setTimeInterval(int ms)
{
    m_queryTimer->setInterval(ms);
}
//================================================================================
void DataQueryEngine::startDataQuery()
{
    ...
    if (std::ifstream(stat_filepath) && std::ifstream("/proc/stat")) isStartSuccessful = true;
    ...
    if ((kret = task_for_pid(current_task(), m_currentPid, &task)) == KERN_SUCCESS)
    {
        ...
        if (KERN_SUCCESS == task_info(task, TASK_ABSOLUTETIME_INFO, (task_info_t)&timeInfo, &info_count))
...
#error "current OS is not supported"
    ...
    if (isStartSuccessful)
    {
        ...
        qWarning() << "RamDataQuery::startDataQuery: start fail";
}
//================================================================================
...
void DataQueryEngine::getData()
{
    ...
    if (GetProcessMemoryInfo(m_hProcess, &pmc, sizeof(pmc)))
    {
        ...
        emit ramDataUpdated(pmc.WorkingSetSize / 1024, pmc.PagefileUsage / 1024, pmc.PeakPagefileUsage / 1024);
    }
}
#elif defined Q_OS_LINUX
void DataQueryEngine::getData()
{
    ...
    stat_stream >> p_pid >> comm >> state >> ppid >> pgrp >> session >> tty_nr >> tpgid >> flags >> minflt >> cminflt >>
        majflt >> cmajflt >> cur_pstat.utime_ticks >> cur_pstat.stime_ticks >> cur_pstat.cutime_ticks >>
        cur_pstat.cstime_ticks >> priority >> nice >> o >> itrealvalue >> starttime >> vsize >> rss;
    ...
    stat_stream_2 >> cpu >> cpu_time[0] >> cpu_time[1] >> cpu_time[2] >> cpu_time[3] >> cpu_time[4] >> cpu_time[5] >>
        cpu_time[6] >> cpu_time[7] >> cpu_time[8] >> cpu_time[9];

    for (int i = 0; i < numOfCpuValues; ++i)
        cur_pstat.cpu_total_time += cpu_time[i];

    // calculate cpu usage
    const long unsigned int total_time_diff = cur_pstat.cpu_total_time - last_usage.cpu_total_time;

    double ucpu_usage =
        100 * (((cur_pstat.utime_ticks + cur_pstat.cutime_ticks) - (last_usage.utime_ticks + last_usage.cutime_ticks)) /
               (double)total_time_diff);

    double scpu_usage =
        100 *
        ((((cur_pstat.stime_ticks + cur_pstat.cstime_ticks) - (last_usage.stime_ticks + last_usage.cstime_ticks))) /
         (double)total_time_diff);
...
void DataQueryEngine::getData()
{
    ...
    if (KERN_SUCCESS == task_info(task, TASK_VM_INFO, (task_info_t)&vm_info, &info_count))
    {
        emit ramDataUpdated(vm_info.resident_size / 1024.0, vm_info.internal / 1024.0,
