#include "imagecomparatorslider.hpp"

#include <QLabel>
#include <QPixmap>
#include <QPaintEvent>

ImageComparatorSlider::ImageComparatorSlider(QWidget* parent)
...
ImageComparatorSlider::~ImageComparatorSlider() = default;

void ImageComparatorSlider::setPixmap1(QPixmap p)
{
    pix1 = p;
    recalculateGeometry();
}

void ImageComparatorSlider::setPixmap2(QPixmap p)
{
    pix2 = p;
}

QSize ImageComparatorSlider::sizeHint() const
{
    return pix1.size();
}

void ImageComparatorSlider::resizeEvent(QResizeEvent* ev)
{
    QSplitter::resizeEvent(ev);
    recalculateGeometry();
}

void ImageComparatorSlider::paintEvent(QPaintEvent* ev)
{
    if (pix1.isNull())
    {
...
void ImageComparatorSlider::recalculateGeometry()
{
    if (pix1.isNull())
    {
        curr_sz = size();
