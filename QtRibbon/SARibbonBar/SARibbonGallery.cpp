...
public:
    ...
    bool isValid() const { return (q_ptr != nullptr); }
...
void SARibbonGalleryViewport::removeWidget(QWidget* w)
{
    ...
    if (label) m_layout->removeWidget(label);
...
void SARibbonGallery::addGalleryGroup(SARibbonGalleryGroup* group)
{
    ...
    connect(group, &SARibbonGalleryGroup::groupTitleChanged, this,
            [group, viewport](const QString& t) { viewport->onTitleChanged(group, t); });
...
{
    SARibbonGalleryGroup* group = RibbonSubElementFactory->createRibbonGalleryGroup(this);
    if (!title.isEmpty()) group->setGroupTitle(title);
...
{
    if (nullptr == d_ptr->mPopupWidget) return;
...
void SARibbonGallery::onTriggered(QAction* action)
{
    ...
    {
        if (d_ptr->mPopupWidget->isVisible()) d_ptr->mPopupWidget->hide();
    }
}

SARibbonGalleryViewport* SARibbonGallery::ensureGetPopupViewPort()
{
    if (nullptr == d_ptr->mPopupWidget) d_ptr->createPopupWidget();
    return (d_ptr->mPopupWidget);
}
