...
{
    if (mAction) return (mAction->text());
...
{
    if (mAction) return (mAction->toolTip());
...
{
    if (mAction) return (mAction->icon());
...
{
    mFlags = flag;
    if (mAction) mAction->setEnabled(flag & Qt::ItemIsEnabled);
}
