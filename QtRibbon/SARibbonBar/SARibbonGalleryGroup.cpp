...
void SARibbonGalleryGroupItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option,
                                             const QModelIndex& index) const
{
    if (nullptr == mGroup) return;
...
void SARibbonGalleryGroupItemDelegate::paintIconOnly(QPainter* painter, const QStyleOptionViewItem& option,
                                                     const QModelIndex& index) const
{
...
void SARibbonGalleryGroupItemDelegate::paintIconWithText(QPainter* painter, const QStyleOptionViewItem& option,
                                                         const QModelIndex& index) const
{
    QStyledItemDelegate::paint(painter, option, index);
}

void SARibbonGalleryGroupItemDelegate::paintIconWithTextWordWrap(QPainter* painter, const QStyleOptionViewItem& option,
                                                                 const QModelIndex& index) const
{
    QStyledItemDelegate::paint(painter, option, index);
}
...
{
    if (!index.isValid() || (index.row() >= mItems.size())) return (Qt::NoItemFlags);
    return (mItems.at(index.row())->flags());
}

QVariant SARibbonGalleryGroupModel::data(const QModelIndex& index, int role) const
{
    if (!index.isValid() || (index.row() >= mItems.count())) return (QVariant());
...
{
    if (hasIndex(row, column, parent)) return (createIndex(row, column, mItems.at(row)));
    return (QModelIndex());
}

bool SARibbonGalleryGroupModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    if (!index.isValid() || (index.row() >= mItems.count())) return (false);
...
void SARibbonGalleryGroupModel::clear()
{
    ...
    for (int i = 0; i < mItems.count(); ++i)
        if (mItems.at(i)) delete mItems.at(i);
    mItems.clear();
    endResetModel();
}
...
{
    if ((row < 0) || (row >= mItems.count())) return (0);
...
//===================================================
SARibbonGalleryGroup::SARibbonGalleryGroup(QWidget* w): QListView(w), d_ptr(new SARibbonGalleryGroup::PrivateData(this))
{
...
{
    if (isRecalcGridSizeBlock()) return;
    ...
    if (h <= 1) h = galleryHeight;
    int w = h;
    if (gridMinimumWidth() > 0)
    {
        if (w < gridMinimumWidth()) w = gridMinimumWidth();
    }
    if (gridMaximumWidth() > 0)
    {
        if (w > gridMaximumWidth()) w = gridMaximumWidth();
    }
...
void SARibbonGalleryGroup::setGalleryGroupStyle(SARibbonGalleryGroup::GalleryGroupStyle style)
{
    ...
    if (style == IconWithWordWrapText) setWordWrap(true);
    recalcGridSize();
}
...
void SARibbonGalleryGroup::selectByIndex(int i)
{
    ...
    if (selmodel) selmodel->select(ind, QItemSelectionModel::SelectCurrent);
}
...
void SARibbonGalleryGroup::onItemEntered(const QModelIndex& index)
{
    if (index.isValid())
    {
        ...
        {
            QAction* act = item->action();
            if (act) act->activate(QAction::Hover);
        }
    }
}
