...
SARibbonPannelLayout::~SARibbonPannelLayout()
{
    ...
    while (!mItems.isEmpty())
    {
        ...
        {
            if (item->customWidget) widgetAction->releaseWidget(item->widget());
        }
        delete item;
    }
}
...
int SARibbonPannelLayout::indexByAction(QAction* action) const
{
    for (int i = 0; i < mItems.count(); ++i)
        if (mItems.at(i)->action == action) return (i);
    return (-1);
}
...
void SARibbonPannelLayout::setOptionAction(QAction* action)
{
    ...
    if (action)
    {
        ...
        if (nullptr == mOptionActionBtn)
        {
            ...
            if (mTitleLabel) mTitleLabel->stackUnder(mOptionActionBtn);
        }
        mOptionActionBtn->setDefaultAction(action);
        if (action->icon().isNull()) mOptionActionBtn->setIcon(QIcon(":/image/resource/ribbonPannelOptionButton.png"));
        // 标记需要重新计算尺寸
        invalidate();
    }
    else
...
{
    if ((index < 0) || (index >= mItems.count())) return (nullptr);
...
SARibbonPannelItem* SARibbonPannelLayout::pannelItem(QAction* action) const
{
    ...
    if (index >= 0) return (mItems[index]);
    return (nullptr);
}
...
{
    if (mItems.isEmpty()) return (nullptr);
    return (mItems.last());
}
...
void SARibbonPannelLayout::doLayout()
{
    ...
    for (SARibbonPannelItem* item : qAsConst(mItems))
    {
        ...
        else
        {
            ...
            if (item->widget()) item->widget()->setGeometry(item->itemWillSetGeometry);
    ...
    if (mTitleLabel)
    {
        if (isEnableShowPannelTitle())
        {
            ...
            if (!mTitleLabel->isVisibleTo(pannel)) mTitleLabel->show();
        }
        else
        {
            if (mTitleLabel->isVisibleTo(pannel)) mTitleLabel->hide();
        }
    }
    // 布局m_optionActionBtn
    if (mOptionActionBtn)
...
SARibbonPannelItem* SARibbonPannelLayout::createItem(QAction* action, SARibbonPannelItem::RowProportion rp)
{
    ...
    if (!widget)
    {
        SARibbonToolButton::RibbonButtonType buttonType =
            ((rp == SARibbonPannelItem::Large) ? SARibbonToolButton::LargeButton : SARibbonToolButton::SmallButton);
...
void SARibbonPannelLayout::updateGeomArray(const QRect& setrect)
{
    ...
    for (int i = 0; i < itemCount; ++i)
    {
        ...
        {
            // 有窗口是水平扩展，则标记为扩展
            if ((item->widget()->sizePolicy().horizontalPolicy() & QSizePolicy::ExpandFlag)) mExpandFlag = true;
        }
    ...
    if (isHaveOptionAction())
    {
        ...
        if (isEnableShowPannelTitle())
        {
            ...
            mOptionActionBtnGeometry.setRect(mTitleLabelGeometry.right() - mTitleLabelGeometry.height(),
                                             mTitleLabelGeometry.y(), mTitleLabelGeometry.height(),
        ...
        else
        {
            ...
            mOptionActionBtnGeometry.setRect(setrect.right() - optBtnSize.width() - mag.right(),
                                             setrect.bottom() - optBtnSize.height() - mag.bottom(), optBtnSize.width(),
                                             optBtnSize.height());
...
void SARibbonPannelLayout::recalcExpandGeomArray(const QRect& setrect)
{
    ...
    for (SARibbonPannelItem* item : qAsConst(mItems))
    {
        if ((!item->isEmpty()) && item->expandingDirections() & Qt::Horizontal)
        {
            ...
            if (i == columnExpandInfo.end()) i = columnExpandInfo.insert(item->columnIndex, _columnExpandInfo());
...
{
    if (mTitleSpace == newTitleSpace) return;
...
{
    if (mTitleHeight == newTitleHeight) return;
