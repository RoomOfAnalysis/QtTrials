...
public:
    ...
    void updateSize() { resizeElement(q_ptr->size()); }
    ...
    QSize sizeHint() const
    {
        ...
        QSize res(0, 0);
        if (mButtonGroup) res = mButtonGroup->sizeHint();
        res.setHeight(height);
        if (buttonClose) res.setWidth(res.width() + closeButtonWidthHint());
        if (buttonMaximize) res.setWidth(res.width() + maxButtonWidthHint());
        if (buttonMinimize) res.setWidth(res.width() + minButtonWidthHint());
        return res;
    }
};
...
Qt::WindowFlags SARibbonSystemButtonBar::windowButtonFlags() const
{
    ...
    if (d_ptr->mFlags & Qt::WindowCloseButtonHint) f |= Qt::WindowCloseButtonHint;
    if (d_ptr->mFlags & Qt::WindowMaximizeButtonHint) f |= Qt::WindowMaximizeButtonHint;
    if (d_ptr->mFlags & Qt::WindowMinimizeButtonHint) f |= Qt::WindowMinimizeButtonHint;
...
bool SARibbonSystemButtonBar::eventFilter(QObject* obj, QEvent* event)
{
    if (obj == parent())
    {
        ...
        if (event->type() == QEvent::Resize)
        {
            ...
            if (th != height()) setWindowTitleHeight(th);
...
QAction* SARibbonSystemButtonBar::addAction(QAction* a, Qt::ToolButtonStyle buttonStyle,
                                            QToolButton::ToolButtonPopupMode popMode)
{
    return d_ptr->mButtonGroup->addAction(a, buttonStyle, popMode);
}

QAction* SARibbonSystemButtonBar::addAction(const QString& text, const QIcon& icon, Qt::ToolButtonStyle buttonStyle,
                                            QToolButton::ToolButtonPopupMode popMode)
{
    return d_ptr->mButtonGroup->addAction(text, icon, buttonStyle, popMode);
}

QAction* SARibbonSystemButtonBar::addMenu(QMenu* menu, Qt::ToolButtonStyle buttonStyle,
                                          QToolButton::ToolButtonPopupMode popMode)
{
...
{
    if (parentWidget()) parentWidget()->showMinimized();
}
