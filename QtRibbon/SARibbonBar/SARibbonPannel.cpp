...
{
    if (SARibbonPannelLayout* lay = pannelLayout()) return (qobject_cast<SARibbonToolButton*>(lay->lastWidget()));
    return (nullptr);
}
...
SARibbonPannelItem::RowProportion SARibbonPannel::getActionRowProportionProperty(QAction* action)
{
    ...
    if (isok) return (static_cast<SARibbonPannelItem::RowProportion>(r));
...
QToolButton::ToolButtonPopupMode SARibbonPannel::getActionToolButtonPopupModeProperty(QAction* action)
{
    ...
    if (isok) return (static_cast<QToolButton::ToolButtonPopupMode>(r));
...
Qt::ToolButtonStyle SARibbonPannel::getActionToolButtonStyleProperty(QAction* action)
{
    ...
    if (isok) return (static_cast<Qt::ToolButtonStyle>(r));
 ...
 */
void SARibbonPannel::addAction(QAction* act, QToolButton::ToolButtonPopupMode popMode,
 ...
 * @return 返回添加的action
 */
QAction* SARibbonPannel::addAction(const QString& text, const QIcon& icon, QToolButton::ToolButtonPopupMode popMode,
                                   SARibbonPannelItem::RowProportion rp)
{
 ...
 * @return
 */
void SARibbonPannel::addMenu(QMenu* menu, SARibbonPannelItem::RowProportion rp,
...
SARibbonGallery* SARibbonPannel::addGallery(bool expanding)
{
    ...
    if (expanding) setExpanding(expanding);
    return (gallery);
}
...
SARibbonToolButton* SARibbonPannel::actionToRibbonToolButton(QAction* action)
{
    ...
    for (auto obj : qAsConst(children()))
    {
        if (obj->isWidgetType())
        {
            ...
            {
                if (btn->defaultAction() == action) return btn;
            }
        }
    }
    return (nullptr);
...
{
    if (d_ptr->m_pannelLayoutMode == mode) return;
...
{
    if (SARibbonPannelLayout* lay = pannelLayout()) lay->setOptionAction(action);
}
...
{
    if (SARibbonPannelLayout* lay = pannelLayout()) return lay->isHaveOptionAction();
    return false;
}
...
{
    if (SARibbonPannelLayout* lay = pannelLayout()) lay->setToolButtonIconSize(s);
...
{
    if (SARibbonPannelLayout* lay = pannelLayout()) return lay->toolButtonIconSize();
    return QSize();
}
...
{
    if (SARibbonPannelLayout* lay = pannelLayout()) lay->setPannelTitleHeight(h);
}
...
{
    if (SARibbonPannelLayout* lay = pannelLayout()) return lay->pannelTitleHeight();
    return -1;
}
...
{
    if (SARibbonPannelLayout* lay = pannelLayout()) return lay->isEnableShowPannelTitle();
    return false;
}
...
{
    if (SARibbonPannelLayout* lay = pannelLayout()) return lay->setEnableShowPannelTitle(on);
}
...
{
    if (SARibbonPannelLayout* lay = pannelLayout()) return (lay->indexByAction(act));
    return (-1);
}
...
{
    if (SARibbonPannelLayout* lay = pannelLayout()) return (lay->move(from, to));
...
void SARibbonPannel::updateItemGeometry()
{
    ...
    resetToolButtonSize();
    if (SARibbonPannelLayout* lay = pannelLayout()) lay->updateGeomArray();
}
...
{
    if (SARibbonCategory* c = category()) return c->ribbonBar();
    return nullptr;
}
...
{
    if (auto lay = layout()) lay->setSpacing(n);
}
...
{
    if (auto lay = layout()) return lay->spacing();
    return 0;
}
...
bool SARibbonPannel::event(QEvent* e)
{
    ...
    if (e->type() != QEvent::Paint) qDebug() << "SARibbonPannel event(" << e->type() << "),name=" << pannelName();
#endif
    // if (SARibbonPannelLayout* lay = pannelLayout()) {
    ...
    case QEvent::ActionAdded:
        {
            ...
            {
                if (widgetAction->parent() != this) widgetAction->setParent(this);
            }
            ...
            if (e->before())
            {
                ...
                if (-1 == index) index = layout()->count(); // 找不到的时候就插入到最后
            }
    ...
    case QEvent::ActionChanged:
        {
            ...
            if (QWidget* parw = parentWidget())
            {
                if (QLayout* pl = parw->layout()) pl->invalidate();
...
void SARibbonPannel::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::FontChange)
    {
        ...
        for (QWidget* w : listWidgets)
            w->setFont(f);
        if (QLayout* lay = layout()) lay->invalidate();
