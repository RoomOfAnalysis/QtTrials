...
bool sa_customize_datas_to_xml(QXmlStreamWriter* xml, const QList<SARibbonCustomizeData>& cds)
{
    ...
    xml->writeEndElement();
    if (xml->hasError()) qWarning() << "write has error";
    return (true);
}
...
QList<SARibbonCustomizeData> sa_customize_datas_from_xml(QXmlStreamReader* xml, SARibbonActionsManager* mgr)
{
    ...
    while (!xml->atEnd())
    {
        ...
        if (xml->isStartElement() && (xml->name().toString() == "sa-ribbon-customize")) break;
        xml->readNext();
    }
    ...
    while (!xml->atEnd())
    {
        if (xml->isStartElement() && (xml->name().toString() == "customize-data"))
        {
            ...
            if (attrs.hasAttribute("index"))
            {
                ...
                if (isOk) d.indexValue = v;
            }
            if (attrs.hasAttribute("key")) d.keyValue = attrs.value("key").toString();
            if (attrs.hasAttribute("category")) d.categoryObjNameValue = attrs.value("category").toString();
            if (attrs.hasAttribute("pannel")) d.pannelObjNameValue = attrs.value("pannel").toString();
            if (attrs.hasAttribute("row-prop"))
            {
                v = xml->attributes().value("row-prop").toInt(&isOk);
                if (isOk) d.actionRowProportionValue = static_cast<SARibbonPannelItem::RowProportion>(v);
            }
...
public:
    ...
    void retranslateUi(QWidget* customizeWidget)
    {
        customizeWidget->setWindowTitle(
            QApplication::translate("SARibbonCustomizeWidget", "Customize Widget", Q_NULLPTR));
        labelSelectAction->setText(
            QApplication::translate("SARibbonCustomizeWidget", "Please Select", Q_NULLPTR)); // cn:请选择
        ...
        radioButtonMainCategory->setText(
            QApplication::translate("SARibbonCustomizeWidget", "Main Category", Q_NULLPTR)); // cn:主选项卡
        radioButtonAllCategory->setText(
            QApplication::translate("SARibbonCustomizeWidget", "All Category", Q_NULLPTR)); // cn:所有选项卡
...
void SARibbonCustomizeWidget::PrivateData::updateModel()
{
    ...
    for (const SARibbonCategory* c : qAsConst(categorys))
    {
        ...
        for (const SARibbonPannel* p : qAsConst(pannels))
        {
            ...
            {
                if (i->action->isSeparator()) continue;
...
QString SARibbonCustomizeWidget::PrivateData::itemObjectName(QStandardItem* item) const
{
    ...
    else
    {
        ...
        if (0 == level)
        {
            ...
            if (category) objName = category->objectName();
        }
        else if (1 == level)
        {
            SARibbonPannel* pannel = itemToPannel(item);
            if (pannel) objName = pannel->objectName();
        }
    }
    return (objName);
}
...
{
    if (2 != itemLevel(item)) return (nullptr);
    ...
    {
        SARibbonPannelItem* pi =
            reinterpret_cast<SARibbonPannelItem*>(item->data(SARibbonCustomizeWidget::PointerRole).value<qintptr>());
        act = (pi->action);
    }
    return (act);
}
...
void SARibbonCustomizeWidget::initConnection()
{
    ...
    connect(ui->pushButtonNewCategory, &QPushButton::clicked, this,
            &SARibbonCustomizeWidget::onPushButtonNewCategoryClicked);
    connect(ui->pushButtonNewPannel, &QPushButton::clicked, this,
            &SARibbonCustomizeWidget::onPushButtonNewPannelClicked);
    ...
    connect(ui->lineEditSearchAction, &QLineEdit::textEdited, this,
            &SARibbonCustomizeWidget::onLineEditSearchActionTextEdited);
...
void SARibbonCustomizeWidget::setupActionsManager(SARibbonActionsManager* mgr)
{
    ...
    if (d_ptr->mActionMgr) d_ptr->mAcionModel->uninstallActionsManager();
    ...
    {
        if (mgr->tagName(tag).isEmpty()) continue;
...
bool SARibbonCustomizeWidget::toXml(QXmlStreamWriter* xml) const
{
    ...
    if (isApplied()) res << d_ptr->mCustomizeDatasApplied;
    if (isCached()) res << d_ptr->mCustomizeDatasCache;
...
bool SARibbonCustomizeWidget::toXml(const QString& xmlpath) const
{
    ...
    if (!f.open(QIODevice::ReadWrite | QIODevice::Truncate | QIODevice::Text)) return (false);
...
QAction* SARibbonCustomizeWidget::selectedAction() const
{
    ...
    if ((nullptr == m) || !m->hasSelection()) return (nullptr);
...
void SARibbonCustomizeWidget::setSelectItem(QStandardItem* item, bool ensureVisible)
{
    ...
    if (ensureVisible) ui->treeViewResult->scrollTo(item->index());
}
...
void SARibbonCustomizeWidget::onPushButtonNewPannelClicked()
{
    ...
    else if (1 == level)
    {
        ...
        if (nullptr == categoryItem) return;
    ...
    SARibbonCustomizeData d = SARibbonCustomizeData::makeAddPannelCustomizeData(
        ni->text(), ni->row(), categoryObjName, SARibbonCustomizeWidget::PrivateData::makeRandomObjName("pannel"));
...
void SARibbonCustomizeWidget::onPushButtonRenameClicked()
{
    ...
    else if (1 == level)
    {
        ...
        SARibbonCustomizeData d =
            SARibbonCustomizeData::makeRenamePannelCustomizeData(text, cateObjName, pannelObjName);
        d_ptr->mCustomizeDatasCache.append(d);
    }
    else
    {
...
void SARibbonCustomizeWidget::onPushButtonAddClicked()
{
    ...
    if ((nullptr == act) || (nullptr == item)) return;
    int level = itemLevel(item);
    ...
    SARibbonCustomizeData d = SARibbonCustomizeData::makeAddActionCustomizeData(
        key, d_ptr->mActionMgr, selectedRowProportion(), categoryObjName, pannelObjName);
...
void SARibbonCustomizeWidget::onPushButtonDeleteClicked()
{
    ...
    if (nullptr == item) return;
    if (!isItemCanCustomize(item)) return;
    int level = itemLevel(item);
    ...
    else if (2 == level)
    {
        ...
        QString key = d_ptr->mActionMgr->key(act);
        if (key.isEmpty() || catObjName.isEmpty() || pannelObjName.isEmpty()) return;

        SARibbonCustomizeData d =
            SARibbonCustomizeData::makeRemoveActionCustomizeData(catObjName, pannelObjName, key, d_ptr->mActionMgr);
        d_ptr->mCustomizeDatasCache.append(d);
    }
    // 执行删除操作
    removeItem(item);
...
void SARibbonCustomizeWidget::onToolButtonUpClicked()
{
    ...
    if ((nullptr == item) || (0 == item->row())) return;
    int level = itemLevel(item);
    ...
    else if (2 == level)
    {
        ...
        SARibbonCustomizeData d = SARibbonCustomizeData::makeChangeActionOrderCustomizeData(
            d_ptr->itemObjectName(categoryItem), d_ptr->itemObjectName(pannelItem), key, d_ptr->mActionMgr, -1);
        d_ptr->mCustomizeDatasCache.append(d);
        int r = item->row();
...
void SARibbonCustomizeWidget::onToolButtonDownClicked()
{
    ...
    if ((nullptr == item) || ((count - 1) == item->row())) return;
    int level = itemLevel(item);
