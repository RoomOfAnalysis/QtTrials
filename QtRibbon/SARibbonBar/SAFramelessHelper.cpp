...
void SAPrivateFramelessWidgetData::updateRubberBandStatus()
{
    ...
    {
        if (NULL == m_pRubberBand) m_pRubberBand = new QRubberBand(QRubberBand::Rectangle);
    }
    else
    {
        delete m_pRubberBand;
...
void SAPrivateFramelessWidgetData::updateCursorShape(const QPoint& gMousePos)
{
    ...
    {
        if (m_bCursorShapeChanged) m_pWidget->unsetCursor();
        return;
    }
...
{
    if (d->m_bWidgetResizable) updateCursorShape(m_pWidget->mapToGlobal(SA_HOVEREVENT_POS_POINT(event)));
    return (false);
}
    ...
    case QEvent::Leave:
        {
            ...
            if (data) return (data->handleWidgetEvent(event));
            break;
        }
...
{
    if (width > 0) SAPrivateFramelessCursorPosCalculator::s_borderWidth = width;
}

void SAFramelessHelper::setTitleHeight(int height)
{
    if (height > 0) SAPrivateFramelessCursorPosCalculator::s_titleHeight = height;
}
