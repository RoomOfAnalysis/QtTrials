...
{
    if (nullptr == pannel) return;
    SARibbonCategoryLayout* lay = qobject_cast<SARibbonCategoryLayout*>(q_ptr->layout());
    if (nullptr == lay) return;
    if (pannel->parentWidget() != q_ptr) pannel->setParent(q_ptr);
    // 同步状态
...
{
    if (SARibbonCategoryLayout* lay = q_ptr->categoryLayout()) return lay->pannelList();
...
void SARibbonCategory::PrivateData::doWheelEvent(QWheelEvent* event)
{
    ...
    if (totalWidth > contentSize.width())
    {
        ...
        if (!numPixels.isNull())
        {
            if (numDegrees.y() < 0) scrollpix = -scrollpix;
        }
    ...
    else
    {
        ...
        event->ignore();
        // 如滚动过就还原
        if (lay->isScrolled()) lay->scroll(0);
    }
}
...
bool SARibbonCategory::event(QEvent* e)
{
    ...
    if (e->type() != QEvent::Paint) qDebug() << "SARibbonCategory event(" << e->type() << "),name=" << categoryName();
#endif
...
{
    if (SARibbonCategoryLayout* lay = categoryLayout()) return lay->pannelByName(title);
    return nullptr;
}
...
{
    if (SARibbonCategoryLayout* lay = categoryLayout()) return lay->pannelByObjectName(objname);
    return nullptr;
}
...
{
    if (SARibbonCategoryLayout* lay = categoryLayout()) return lay->pannelByIndex(index);
    return nullptr;
}
...
{
    if (SARibbonCategoryLayout* lay = categoryLayout()) return lay->pannelIndex(p);
    return -1;
}
...
void SARibbonCategory::movePannel(int from, int to)
{
    if (from == to) return;
    if (to < 0) to = 0;
    if (to >= pannelCount()) to = pannelCount() - 1;
    if (SARibbonCategoryLayout* lay = categoryLayout()) lay->movePannel(from, to);
}
...
{
    if (SARibbonCategoryLayout* lay = categoryLayout()) return lay->sizeHint();
...
{
    if (SARibbonCategoryLayout* lay = categoryLayout()) return lay->pannelCount();
    return -1;
}
...
SARibbonBar* SARibbonCategory::ribbonBar() const
{
    ...
    if (QWidget* par = parentWidget())
    {
        ...
        while (par)
        {
            if (SARibbonBar* ribbon = qobject_cast<SARibbonBar*>(par)) return ribbon;
            par = par->parentWidget();
        }
    }
    return nullptr;
...
void SARibbonCategory::setCategoryAlignment(SARibbonAlignment al)
{
    ...
    if (lay) lay->setCategoryAlignment(al);
}
...
{
    SARibbonCategoryLayout* lay = qobject_cast<SARibbonCategoryLayout*>(layout());
    if (lay) return lay->categoryAlignment();
...
void SARibbonCategory::setPannelToolButtonIconSize(const QSize& s)
{
    ...
    iterate([s](SARibbonPannel* pannel) -> bool {
        if (pannel) pannel->setToolButtonIconSize(s);
        return true;
    });
}
