...
public:
    ...
    bool operator==(const SARibbonContextCategory* contextPage) { return (this->contextCategory == contextPage); }
};
...
public:
    ...
    std::unique_ptr<int>
        mUserDefTitleBarHeight; ///< 用户定义的标题栏高度，正常不使用用户设定的高度，而是使用自动计算的高度
    std::unique_ptr<int>
        mUserDefTabBarHeight; ///< 用户定义的tabbar高度，正常不使用用户设定的高度，而是使用自动计算的高度
    std::unique_ptr<int>
        mUserDefCategoryHeight; ///< 用户定义的Category的高度，正常不使用用户设定的高度，而是使用自动计算的高度
public:
    ...
    // 计算tabbar高度
    static int calcMainBarHeight(int tabHegith, int titleHeight, int categoryHeight, bool tabOnTitle,
...
void SARibbonBar::PrivateData::init()
{
    ...
    q_ptr->connect(mRibbonTabBar.data(), &QTabBar::tabBarDoubleClicked, q_ptr,
                   &SARibbonBar::onCurrentRibbonTabDoubleClicked);
    ...
    q_ptr->connect(mStackedContainerWidget.data(), &SARibbonStackedWidget::hidWindow, q_ptr,
...
int SARibbonBar::PrivateData::calcTabBarHeight()
{
    ...
    int fontHeight =
        q_ptr->fontMetrics().lineSpacing(); // 不要用height，像宋体这种字体，height=12，lineSpacing=14，有些就无法显示
...
{
    int textH = q_ptr->fontMetrics()
                    .lineSpacing(); // 这里用linespace，因为在换行的情况下，行距是不可忽略的，ribbon的大按钮默认是2行
 ...
 * @return
 */
int SARibbonBar::PrivateData::calcMainBarHeight(int tabHegith, int titleHeight, int categoryHeight, bool tabOnTitle,
                                                SARibbonBar::RibbonMode rMode)
{
...
{
    return calcMainBarHeight(tabBarHeigth(), titleBarHeight(), categoryHeight(), mIsTabOnTitle,
...
{
    if (mUserDefTitleBarHeight) return *mUserDefTitleBarHeight;
...
{
    if (mUserDefTabBarHeight) return *mUserDefTabBarHeight;
...
void SARibbonBar::PrivateData::setApplicationButton(QAbstractButton* btn)
{
    ...
    if (btn)
    {
        if (btn->parent() != q_ptr) btn->setParent(q_ptr);
...
bool SARibbonBar::PrivateData::isContainContextCategoryInList(SARibbonContextCategory* contextCategory)
{
    for (int i = 0; i < mCurrentShowingContextCategory.size(); ++i)
        if (mCurrentShowingContextCategory[i] == contextCategory) return (true);
    return (false);
}
        ...
        for (int i = 0; i < cd.contextCategory->categoryCount(); ++i)
        {
            ...
            for (int t = 0; t < tabcount; ++t)
            {
                ...
                if (v.isValid())
                {
                    ...
                    if (d.category == category) cd.tabPageIndex.append(t);
                }
                else
                {
...
{
    if (h - 8 >= 20) return QSize(h - 8, h - 8);
...
void SARibbonBar::setApplicationButton(QAbstractButton* btn)
{
    ...
    if (btn)
    {
        if (btn->objectName().isEmpty()) btn->setObjectName(QStringLiteral("SARibbonApplicationButton"));
        btn->setVisible(true);
...
SARibbonCategory* SARibbonBar::categoryByName(const QString& title) const
{
    ...
    for (int i = 0; i < c; ++i)
    {
        ...
        if (w)
        {
            if (w->categoryName() == title) return (w);
        }
    }
    return (nullptr);
}
...
SARibbonCategory* SARibbonBar::categoryByObjectName(const QString& objname) const
{
    ...
    for (int i = 0; i < c; ++i)
    {
        ...
        if (w)
        {
            if (w->objectName() == objname) return (w);
        }
    }
    return (nullptr);
}
...
void SARibbonBar::removeCategory(SARibbonCategory* category)
{
    ...
    // 这时要刷新所有tabdata的index信息
    if (isupdate) d_ptr->updateTabData();
    // 移除完后需要重绘
    repaint();
    QApplication::postEvent(this, new QResizeEvent(size(), size()));
}
...
void SARibbonBar::addContextCategory(SARibbonContextCategory* context)
{
    ...
    connect(context, &SARibbonContextCategory::categoryTitleChanged, this,
            &SARibbonBar::onContextsCategoryCategoryNameChanged);
...
{
    if (isContextCategoryVisible(context)) return;
...
void SARibbonBar::destroyContextCategory(SARibbonContextCategory* context)
{
    if (nullptr == context) return;
    //! 1、如果上下文标签显示中，先隐藏
    if (isContextCategoryVisible(context)) hideContextCategory(context);
    //! 2、删除上下文标签的相关内容
...
void SARibbonBar::setTabBarHeight(int h, bool resizeByNow)
{
    ...
    if (resizeByNow) updateRibbonGeometry();
}
...
{
    if (quickAccessBar()) quickAccessBar()->setIcon(i);
}
    ...
    if (isMinimumMode())
    {
        ...
        if (!d_ptr->mStackedContainerWidget->isVisible())
        {
            if (d_ptr->mStackedContainerWidget->isPopupMode())
            {
                ...
                QHoverEvent ehl(QEvent::HoverLeave, d_ptr->mRibbonTabBar->mapToGlobal(QCursor::pos()),
...
int SARibbonBar::tabIndex(SARibbonCategory* obj)
{
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        if (v.isValid())
        {
            ...
            if (innervalue.category == obj) return (i);
        }
    }
...
void SARibbonBar::setPannelSpacing(int n)
{
    ...
    iterate([n](SARibbonCategory* category) -> bool {
        if (category) category->setPannelSpacing(n);
        return true;
    });
}
...
void SARibbonBar::setPannelToolButtonIconSize(const QSize& s)
{
    ...
    iterate([s](SARibbonCategory* category) -> bool {
        if (category) category->setPannelToolButtonIconSize(s);
        return true;
    });
}
...
{
    if (QWidget* w = cornerWidget(c)) w->setVisible(on);
}
...
void SARibbonBar::paintInLooseStyle()
{
    ...
    if (parWindow)
    {
        ...
        {
            titleRegion.setRect(d_ptr->mQuickAccessBar->geometry().right() + 1, border.top(),
        ...
        else
        {
            ...
            else
            {
                ...
                titleRegion.setRect(d_ptr->mIconRightBorderPosition + d_ptr->mQuickAccessBar->geometry().right(),
                                    border.top(), leftwidth, titleBarHeight());
            }
        }
#ifdef SA_RIBBON_DEBUG_HELP_DRAW
        p.save();
 ...
 */
void SARibbonBar::paintContextCategoryTab(QPainter& painter, const QString& title, const QRect& contextRect,
                                          const QColor& color)
{
    // 绘制上下文标签
    // 首先有5像素的实体粗线位于顶部
    ...
    if (isLooseStyle())
    {
        if (!title.isEmpty())
        {
            QRect textRect = QRect(contextRect.x(), contextRect.y() + contextLineWidth, contextRect.width(),
...
void SARibbonBar::resizeInCompactStyle()
{
    ...
    if (ribbonAlignment() == SARibbonAlignment::AlignLeft)
    {
        if (mintabBarWidth < tabBarAllowedWidth) tabBarAllowedWidth = mintabBarWidth;
        d_ptr->mRibbonTabBar->setGeometry(x, y, tabBarAllowedWidth, tabH);
    }
    else
    {
...
{
    if (!d_ptr->mTabBarBaseLineColor.isValid()) return;
    painter.save();
    // 在tabbar下绘制一条线
...
void SARibbonBar::paintWindowTitle(QPainter& painter, const QString& title, const QRect& titleRegion)
{
    ...
    painter.save();
    if (d_ptr->mTitleTextColor.isValid()) painter.setPen(d_ptr->mTitleTextColor);
