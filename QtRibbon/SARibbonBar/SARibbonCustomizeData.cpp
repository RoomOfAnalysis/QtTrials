...
SARibbonCustomizeData::SARibbonCustomizeData()
    : indexValue(-1), actionRowProportionValue(SARibbonPannelItem::Large), mType(UnknowActionType),
...
bool SARibbonCustomizeData::apply(SARibbonBar* bar) const
{
    ...
    case AddActionActionType:
        {
            if (nullptr == mActionsManagerPointer) return (false);
            SARibbonCategory* c = bar->categoryByObjectName(categoryObjNameValue);
            if (nullptr == c) return (false);
            SARibbonPannel* pannel = c->pannelByObjectName(pannelObjNameValue);
            if (nullptr == pannel) return (false);
    ...
    case ChangeCategoryOrderActionType:
        {
            ...
            if (-1 == currentindex) return (false);
 ...
 */
SARibbonCustomizeData SARibbonCustomizeData::makeAddCategoryCustomizeData(const QString& title, int index,
 ...
 */
SARibbonCustomizeData SARibbonCustomizeData::makeAddPannelCustomizeData(const QString& title, int index,
 ...
 * @return
 */
SARibbonCustomizeData SARibbonCustomizeData::makeAddActionCustomizeData(const QString& key, SARibbonActionsManager* mgr,
...
QList<SARibbonCustomizeData> remove_indexs(const QList<SARibbonCustomizeData>& csd, const QList<int>& willremoveIndex)
{
    ...
    for (int i = 0; i < csd.size(); ++i)
        if (!willremoveIndex.contains(i)) res << csd[i];
    return (res);
}
...
QList<SARibbonCustomizeData> SARibbonCustomizeData::simplify(const QList<SARibbonCustomizeData>& csd)
{
    ...
    for (int i = 1; i < size; ++i)
    {
        ...
        {
            if (csd[i - 1].categoryObjNameValue == csd[i].categoryObjNameValue) willremoveIndex << i - 1 << i;
        }
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        {
            if (0 == res[i].indexValue) willremoveIndex << i;
        }
    }
    res = remove_indexs(res, willremoveIndex);
    willremoveIndex.clear();
