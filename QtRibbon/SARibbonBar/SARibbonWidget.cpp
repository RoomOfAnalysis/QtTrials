...
{
    QLayout* lay = layout();
    if (lay) return qobject_cast<SARibbonBar*>(lay->menuBar());
    return nullptr;
}
...
void SARibbonWidget::setWidget(QWidget* w)
{
    ...
    if (QLayout* lay = layout()) lay->addWidget(w);
}
