...
void SARibbonMainWindow::setRibbonBar(SARibbonBar* ribbon)
{
    ...
    if (sysBar->closeButton()) helper->setSystemButton(QWK::WindowAgentBase::Close, sysBar->closeButton());
    if (sysBar->minimizeButton()) helper->setSystemButton(QWK::WindowAgentBase::Minimize, sysBar->minimizeButton());
    if (sysBar->maximizeButton()) helper->setSystemButton(QWK::WindowAgentBase::Maximize, sysBar->maximizeButton());
#endif
#else
    // 捕获ribbonbar的事件
    ...
    // 最后要提升，否则新加入的会被覆盖
    if (d_ptr->mWindowButtonGroup) d_ptr->mWindowButtonGroup->raise();
}
...
{
    if (SARibbonSystemButtonBar* g = d_ptr->mWindowButtonGroup) g->updateWindowFlag(flags);
}
    ...
    if (SARibbonBar* bar = ribbonBar())
    {
        ...
        case SARibbonTheme::RibbonThemeOffice2021Blue:
            bar->setContextCategoryColorList(QList<QColor>()
                                             << QColor(209, 207, 209)); //< 设置空颜色列表会重置为默认色系
            break;
        default:
            break;
        }
...
bool SARibbonMainWindowEventFilter::eventFilter(QObject* obj, QEvent* e)
{
    if (e)
    {
        if (e->type() == QEvent::Resize)
        {
            ...
            {
                if (SARibbonBar* ribbon = m->ribbonBar()) ribbon->setFixedWidth(m->size().width());
            }
        }
    }
