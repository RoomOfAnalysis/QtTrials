...
void SARibbonActionsManager::removeTag(int tag)
{
    ...
    for (QAction* a : qAsConst(oldacts))
        if (!total.contains(a)) needRemoveAct.append(a);
    // 从总表移除action
...
{
    if (nullptr == act) return (false);
    QString k = key;

    if (k.isEmpty()) k = QString("id_%1_%2").arg(d_ptr->mSale++).arg(act->objectName());
    if (d_ptr->mKeyToAction.contains(k))
    {
        qWarning()
            << "key: " << k
            << " have been exist,you can set key in an unique value when use SARibbonActionsManager::registeAction";
        return (false);
    }
...
QMap<int, SARibbonCategory*> SARibbonActionsManager::autoRegisteActions(SARibbonBar* bar)
{
    ...
    if (parWidget)
    {
        for (QObject* o : qAsConst(parWidget->children()))
        {
            ...
            {
                // 说明是action
                if (!a->objectName().isEmpty()) mainwindowActions.insert(a);
            }
        }
    }
    ...
    for (QAction* a : qAsConst(notincategory))
        if (!a->objectName().isEmpty()) registeAction(a, NotInRibbonCategoryTag, a->objectName(), false);
    if (notincategory.size() > 0) setTagName(NotInRibbonCategoryTag, tr("not in ribbon"));
    for (auto i = res.begin(); i != res.end(); ++i)
    {
        connect(i.value(), &SARibbonCategory::categoryNameChanged, this,
                &SARibbonActionsManager::onCategoryTitleChanged);
    }
...
QSet<QAction*> SARibbonActionsManager::autoRegisteWidgetActions(QWidget* w, int tag, bool enableEmit)
{
    ...
    for (QAction* a : qAsConst(was))
    {
        ...
        if (registeAction(a, tag, a->objectName(), enableEmit)) res.insert(a);
    }
    return (res);
}
...
QList<QAction*> SARibbonActionsManager::search(const QString& text)
{
    ...
    if (kws.isEmpty()) kws.append(text);
    ...
    for (const QString& k : qAsConst(kws))
    {
        for (auto i = d_ptr->mActionToKey.begin(); i != d_ptr->mActionToKey.end(); ++i)
            if (i.key()->text().contains(k, Qt::CaseInsensitive)) res.append(i.key());
    }
    return (res);
}
...
QVariant SARibbonActionsManagerModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    ...
    if (Qt::Horizontal == orientation) return (tr("action name"));
    return (QVariant());
}
...
QVariant SARibbonActionsManagerModel::data(const QModelIndex& index, int role) const
{
    ...
    if (nullptr == act) return (QVariant());
    ...
    case Qt::DisplayRole:
        {
            ...
            {
                if (QWidget* w = wa->defaultWidget()) str = w->windowTitle();
            }
            return str;
        }
...
void SARibbonActionsManagerModel::uninstallActionsManager()
{
    if (!d_ptr->isNull())
    {
        disconnect(d_ptr->mMgr, &SARibbonActionsManager::actionTagChanged, this,
                   &SARibbonActionsManagerModel::onActionTagChanged);
        d_ptr->mMgr = nullptr;
        d_ptr->mTag = SARibbonActionsManager::CommonlyUsedActionTag;
    }
    update();
...
QAction* SARibbonActionsManagerModel::indexToAction(QModelIndex index) const
{
    ...
    if (index.row() >= d_ptr->count()) return (nullptr);
